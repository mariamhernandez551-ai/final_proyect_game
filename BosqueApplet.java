...
/**
 * Proyecto Final Estructura de Datos: Bosque de los Susurros.
 * Implementación de un juego de aventura basado en cuadrículas usando Applet.
...
public class BosqueApplet extends Applet implements KeyListener {
    ...
    public void init() {
        ...
        mensajesCola.offer("Tu objetivo es encontrar el Hongo Dorado (código 6).");
    }

    /**
     * Lógica de dibujo principal. Se llama con repaint().
     */
    public void paint(Graphics g) {
        ...
        g.drawString("POSICIÓN (Array 1D): (" + aventureroX + ", " + aventureroY + ")", 10, panelY + 60);
    ...
    /**
     * Lógica para procesar el movimiento y las interacciones.
            ...
            if (destino != 1) {
                ...
                switch (destino) {
                    case 0: // Pasto (Movimiento normal)
                        mensajeActual = "Te has movido a un claro vacío.";
                        break;
                    case 2: // Hongo Parlante (NPC) - Usa Cola y Lista
                        if (!mensajesCola.isEmpty()) {
                            mensajeActual = "🍄 Hongo dice: " + mensajesCola.poll();
                        } else {
                            mensajeActual = "Hongo: 'No tengo más que decir, ¡sigue tu camino!'";
                        }
                        if (inventario.size() < 4) {
                            inventario.add("Poción de Diálogo"); // Añadimos a la Lista de Inventario
                        }
                        break;
                    case 3: // Tesoro (Ingrediente Mágico) - Usa Lista
                        String nuevoItem = "Ingrediente Mágico (" + inventario.size() + ")";
                        ...
                        break;
                    case 4: // Enemigo (Planta Pícara) - Usa Array 1D para salud
                        salud -= 20; // Reduce salud
                        mensajeActual = "¡Planta Pícara! Te ha quitado 20 de salud. Salud actual: " + salud;
                        if (salud <= 0) {
                            juegoTerminado = true;
                            mensajeActual = "HAS PERDIDO. Te quedaste sin salud.";
                        } else {
                            // Limpiamos el enemigo después del "combate"
                            destino = 0; 
                        }
                        break;
                    case 6: // Meta (Hongo Dorado)
                ...
                // 4. Actualizar la posición del Aventurero en el Array 1D y Matriz
                aventureroX = nuevoX;
                aventureroY = nuevoY;
                ...
                mensajeActual = "¡Movimiento bloqueado! Hay un árbol en el camino.";
            }
        }
    }
    
    /**
     * Implementa la función Deshacer usando la Pila.
     */
    private void deshacerMovimiento() {
        ...
        // 1. Sacar la posición anterior de la Pila (LIFO)
        Point posicionPrevia = historialMovimiento.pop();
        
        // 2. Limpiar la posición actual en la Matriz
        mapa[aventureroY][aventureroX] = 0;

        // 3. Restaurar la posición del Aventurero
        aventureroX = posicionPrevia.x;
        aventureroY = posicionPrevia.y;
        
        // 4. Actualizar la Matriz con la posición restaurada
        mapa[aventureroY][aventureroX] = 5;
    ...
    // ----------------------------------------------------
    // Implementación de KeyListener para el movimiento y comandos
    // ----------------------------------------------------
        ...
        switch (e.getKeyCode()) {
            ...
            case KeyEvent.VK_ENTER: // Comando para avanzar en la cola de mensajes
                if (!mensajesCola.isEmpty()) {
                    ...
                    mensajeActual = "No hay más mensajes en la Cola.";
                }
                return; 
